...
void initHeaps(void) { // 8007B3A4
  ...
  pFrameBuffer_80398b70 = (void*)OSRoundUp32B((u32)pFrameBuffer_80398b74 + iVar1);
...
int _mmGetHeapIdx(void *offset) { //8007C0DC
  ...
  while( true ) {
    ...
    if ((heaps[iHeap].data < offset) &&
       (offset < (void *)((int)&((heaps[iHeap].data)->entry).loc + heaps[iHeap].size))) break;
    iHeap = iHeap + 1;
  }
  return iHeap;
}
...
void _mmActuallyFree(int iHeap,int iEntry) { //8007C154
  ...
  if ((iVar6 != -1) && (entry[iVar6].type == 0)) {
    entry[iEntry].entry.size = entry[iEntry].entry.size + entry[iVar6].entry.size;
  ...
  if ((iVar5 != -1) && (entry[iVar5].type == 0)) {
    entry[iVar5].entry.size = entry[iVar5].entry.size + entry[iEntry].entry.size;
    sVar1 = entry[iEntry].next;
...
undefined * getHeapData(int param_1) { //8007C310
  return (undefined *)heaps[param_1].data;
}
...
int heapSetEntry(int iHeap,int iEntry,u32 size,s16 type,int type2,undefined4 param6) { //8007C328
  ...
  if ((int)size < (int)uVar4) {
    ...
    if (((idx == -1) || (entry[idx].type != 0)) || (type2 != 0)) {
      ...
      idx = (int)sVar1;
      entry[idx].entry.loc = (void *)((int)entry[iEntry].entry.loc + size);
