...
IGNORE_SYMBOLS = (
    ...
    0x802cef61, # part of @stringBase0
    0x802cef66, # part of @stringBase0
    0x802cef6a, # part of @stringBase0
    0x802E175B, # something thinks this byte is a word
    0x802EE505, # something thinks this byte is a word
    0x80325DCC,
    0x80325DD0,
...
def isSymbolALocalLabel(sym):
    ...
    return (func is not None and func.body.minAddress != sym.getAddress())
...
def readSymbolsTxt(path):
    ...
    with open(path, 'r') as file:
        for line in file.readlines():
            ...
            func = getFunctionAt(intToAddr(addr))
            if lbl is not None and lbl != name:
                if func is not None:
                    try: ns = func.getParentNamespace()
            ...
            if name.startswith('s_') or name.startswith('u_'):
                # XXX shouldn't be here, but somehow these
                # keep ending up in the file
            ...
            printf("symtxt: 0x%X %s %s 0x%X", addr, sec, name,
                sym.get('size', 0))
            if (func is None and 'size' in sym and sym['size'] > 0
            and sym['size'] <= 8 and sym['address'] % sym['size'] != 0):
                printf("symtxt: 0x%X %s is not aligned to %d",
                    addr, name, sym['size'])
                del symbols[name]
                clearListing(intToAddr(addr), intToAddr(addr+sym['size']))
    return symbols
...
def applySymbolsTxt(symbols):
    for name, sym in symbols.items():
        ...
        if unit is not None: # something exists here already
                ...
                if name != oldName and (
                    ...
                    or re.match(r'[0-9a-fA-F]+$', oldName))):
                        ...
                        if not oldName.startswith(GENERIC_FUNC_NAME_PREFIXES):
                            ...
                            if type(comment) is unicode: comment = str(comment)
...
def writeSymbolsTxt(outPath, files, symbols):
    ...
    while True:
        ...
        if symName.startswith(('@', 'jumptable_', 'LAB_')):
            continue # these get filtered always
        if symName.startswith(COMMON_PREFIXES):
            # these get filtered only if they overlap
            overlap, ovlAddr, ovlEnd = getOverlappingSymbol(
                addr, symSize+1)
            if overlap is not None and ovlAddr != addr:
                printf("symtxt: ignoring generic symbol '%s' (0x%X) due to overlap at 0x%X",
                    symName, addr, ovlAddr)
        ...
        if symSize > 0 and symSize <= 8 and addr % symSize != 0:
            printf("symtxt write: 0x%X %s is not aligned to %d",
                addr, name, symSize)
        else:
            overlap, ovlAddr, ovlEnd = getOverlappingSymbol(
                addr, symSize+1)
            ...
            and refAddr <  SYMBOL_MAX_ADDR):
                ...
                if sec in ('bss', 'data', 'rodata', 'init'):
                    ...
                    if data is None: dLen = 4
                        ...
                        if data.getBaseDataType().getName() == 'string':
                            # align end address
                            pad = dLen % 4
                            if pad > 0: dLen += 4-pad
                    for dAddr in range(refAddr, refAddr+dLen):
                        ...
                        and func['file'] not in DATA_REFS[dAddr]):
                            ...
                            printf("Func 0x%X '%s' ref data '%s' 0x%X (0x%X - 0x%X, %s, len 0x%X)",
                                func['start'], func['name'], dName,
                                oldAddr, refAddr, refAddr+dLen, sec, dLen)
                else:
        ...
        for secName in SECTION_NAMES:
            ...
            while True:
                ...
                if data is not None:
                    ...
                    if data.getBaseDataType().getName() == 'string':
                        # align end address
                        # maybe doesn't work because getDataContaining
                        # won't catch this padded region.
                        # should instead change the actual data addresses.
                        pad = dEnd % 4
                        if pad > 0: dEnd += 4-pad
                    if sec['max'] > dStart:
                        prevMax2 = sec['max']
                ...
                aMin = intToAddr(sec['min'])
                data = getDataContaining(aMin)
                if data is not None and data.getMinAddress() == aMin:
                    data = None # ignore data if not partial overlap
                wasStart = (data is not None)
                if data is not None:
                    size   = data.getLength()
                    dStart = addrToInt(data.getMinAddress())
                    dEnd   = addrToInt(data.getMaxAddress())
                    ...
                    if sec['min'] > dStart:
                        ...
                        sec['min'] = dStart
                        pad = sec['min'] % align
                        if pad > 0: sec['min'] -= align-pad
                        printf("Section '%s.%s' (0x%X - 0x%X) overlaps data at 0x%X - 0x%X, start moved to 0x%X",
                            fileName, secName, sec['min'], prevMin2,
                            dStart, dEnd, sec['min'])
                    else: wasStart = False

                if not (wasEnd or wasStart): break
...
def padStrings():
    """Pad strings' endings to proper alignment.

    We do this because if a string's terminating null byte
    is at an aligned address, that byte isn't considered
    part of the string, which throws off our alignment
    calculations.
    """
    monitor.setIndeterminate(False)
    monitor.setMessage("Fixing string alignment")
    monitor.setProgress(0)
    monitor.setMaximum(SYMBOL_MAX_ADDR - SYMBOL_MIN_ADDR)
    nextAddr = intToAddr(SYMBOL_MIN_ADDR)
    prevAddr = addrToInt(nextAddr)
    while True:
        sym = None
        while sym is None and addrToInt(nextAddr) < SYMBOL_MAX_ADDR:
            sym = getSymbolAt(nextAddr)
            nextAddr = nextAddr.add(1)
        if sym is None: break
        addrObj = sym.getAddress()
        addr = addrToInt(addrObj)
        monitor.setProgress(addr - SYMBOL_MIN_ADDR)
        # ignore strings that don't start at an aligned address.
        # these are probably embedded in some larger structure
        # or are false positives.
        if (addr & 3) != 0: continue

        data = getDataAt(addrObj)
        if (data is None or data.getDataType().getName()
        not in ('string', 'TerminatedCString')):
            continue

        # get address of the first null byte.
        # the length isn't reliable since we may
        # have previously already padded this string.
        # but, we do need to record it in case we run
        # into a conflict and need to remake it.
        oldLen = data.getLength()
        dLen = 0
        while dLen < 10000:
            b = getByte(addrObj.add(dLen))
            if b == 0: break
            dLen += 1

        # pad to consume following null bytes
        aEnd = addrObj.add(dLen)
        for i in range(8):
            aEnd = addrObj.add(dLen + i)
            # if we hit a label or non-zero byte, stop.
            lbl = getLabelAt(aEnd)
            if lbl is not None: break
            b = getByte(addrObj.add(dLen + i))
            if b != 0: break
        if aEnd == addrObj.add(dLen):
            printf("0x%X - 0x%X no pad str", addr, addrToInt(aEnd))
        else:
            try:
                clearListing(addrObj, addrObj.add(dLen))
                createAsciiString(addrObj, addrToInt(aEnd) - addr)
                printf("0x%X - 0x%X pad str -> 0x%X", addr,
                    addr+dLen, addrToInt(aEnd))
            except ghidra.program.model.util.CodeUnitInsertionException:
                printf("0x%X - 0x%X FAILED pad str -> 0x%X", addr,
                    addr+dLen, addrToInt(aEnd))
                # recreate the string we cleared
                createAsciiString(addrObj, oldLen)
